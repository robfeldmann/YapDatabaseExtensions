...
extension ReadTransactionType {
    ...
    */
    public func readWithMetadataAtIndex<
        ...
        Metadata: NSCoding {
            guard
                let jsonObject = readAtIndex(index),
                let item = try? Value(from: jsonObject) else { return nil }
    ...
    public func writeWithMetadata<
        ...
        Metadata: NSCoding {
            if let jsonObject = try? item.value.jsonObject() {
                writeAtIndex(item.value.index, object: jsonObject, metadata: item.metadata)
            }
            return item
    }
